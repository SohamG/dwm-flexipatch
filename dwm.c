...
#endif // WINDOWROLERULE_PATCH
				&& (!r->instance || strstr(instance, r->instance)) && (!r->wintype || wintype == XInternAtom(dpy, r->wintype, False)))
		{
#if CENTER_PATCH
...
#endif // KEYMODES_PATCH
{
	...
	if (!selmon->sel)
		for (i = 0; i < LENGTH(on_empty_keys); i++)
			if (*keysym == on_empty_keys[i].keysym && CLEANMASK(on_empty_keys[i].mod) == CLEANMASK(ev->state) && on_empty_keys[i].func)
...
void manage(Window w, XWindowAttributes *wa)
{
	...
	c->y = MAX(c->y, ((!c->mon->bar || c->mon->bar->by == c->mon->my) && (c->x + (c->w / 2) >= c->mon->wx) && (c->x + (c->w / 2) < c->mon->wx + c->mon->ww)) ? bh : c->mon->my);
...
#endif // DECK_LAYOUT
#if FLEXTILE_DELUXE_LAYOUT
			 || (&flextile == c->mon->lt[c->mon->sellt]->arrange && ((c->mon->ltaxis[LAYOUT] == NO_SPLIT &&
...
void restack(Monitor *m)
{
	...
	if (m == selmon && (m->tagset[m->seltags] & m->sel->tags) && (
																																	 ...
																																	 (m->lt[m->sellt]->arrange != &monocle && !(m->ltaxis[MASTER] == MONOCLE && (abs(m->ltaxis[LAYOUT] == NO_SPLIT || !m->nmaster || n <= m->nmaster))))
#elif MONOCLE_LAYOUT
